...
# Find all changed package.json files between HEAD~1 and HEAD
# These represent packages that got version bumps
...
for pkg_path in $CHANGED_PKGS; do
  ...
  INCLUDE_JSON+="{\"component\":\"$COMPONENT\",\"version\":\"$NEW_VER\",\"dockerfile\":\"$DOCKERFILE\",\"sentry_project\":\"$SENTRY_PROJECT\"}"
