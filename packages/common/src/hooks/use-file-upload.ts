...
export const useFileUpload = (
  ...
  // Sync state when initialFiles changes (for controlled usage)
  useEffect(() => {
    setState((prev) => ({
      ...prev,
      files: initialFiles.map((file) => ({
  ...
  const clearFiles = useCallback(() => {
    setState((prev) => {
      ...
      setTimeout(() => onFilesChange?.(newState.files), 0);
      return newState;
    });
  }, [onFilesChange]);
    ...
    (newFiles: FileList | File[]) => {
      ...
      setState((prev) => {
        ...
        if (
          multiple &&
          maxFiles !== Infinity &&
          clearedErrorsState.files.length + newFilesArray.length > maxFiles
        ) {
